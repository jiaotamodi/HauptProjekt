...
public class Block
{
	enum Cubeside {BOTTOM, TOP, LEFT, RIGHT, FRONT, BACK,DIAGFRONT, DIAGBACK, DIAGLEFT,DIAGRIGHT };
    ...
    Vector2[,] blockUVs = {
        ///*Transparent*/     {new Vector2 (0,0.0625f), new Vector2(0.0625f,0.0625f),
        //                        new Vector2(0,0.125f), new Vector2(0.0625f,0.125f)},
	...
	private void CreateQuad(Cubeside side)
    {
        ...
        //NO collide vertices
        Vector3[] noCollVertices = new Vector3[4];
        int[] noCollTriangles = new int[6];
        List<Vector2> noCollUVs = new List<Vector2>();

        Mesh noCollMesh = new Mesh();//No collier
        ...
        GameObject noCollQuad = new GameObject("NoCollQuad");
        noCollQuad.transform.position = position;
        noCollQuad.transform.parent = this.parent.transform;
        MeshFilter noCollMeshFilter = (MeshFilter)noCollQuad.AddComponent<MeshFilter>();
        noCollMeshFilter.mesh = noCollMesh;
	...
	public void Draw()
	{
        ...
        if(blockType == BlockType.FLOWER)
        {
            if (!HasSolidNeighbour((int)position.x, (int)position.y, (int)position.z + 1))
                CreateFlowerQuad(Cubeside.FRONT);
            if (!HasSolidNeighbour((int)position.x, (int)position.y, (int)position.z - 1))
                CreateFlowerQuad(Cubeside.BACK);
            if (!HasSolidNeighbour((int)position.x, (int)position.y + 1, (int)position.z))
                CreateFlowerQuad(Cubeside.TOP);
            if (!HasSolidNeighbour((int)position.x, (int)position.y - 1, (int)position.z))
                CreateFlowerQuad(Cubeside.BOTTOM);
            if (!HasSolidNeighbour((int)position.x - 1, (int)position.y, (int)position.z))
                CreateFlowerQuad(Cubeside.LEFT);
            if (!HasSolidNeighbour((int)position.x + 1, (int)position.y, (int)position.z))
                CreateFlowerQuad(Cubeside.RIGHT);
            CreateFlowerQuad(Cubeside.DIAGFRONT);
            CreateFlowerQuad(Cubeside.DIAGBACK);
            CreateFlowerQuad(Cubeside.DIAGLEFT);
            CreateFlowerQuad(Cubeside.DIAGRIGHT);
    ...
    private void CreateFlowerQuad(Cubeside side)
    {
        Mesh mesh = new Mesh();
        mesh.name = "ScriptedMesh" + side.ToString();
        ...
        Vector2[] noCollUVs = new Vector2[4];
        ...
        switch (side)
        {
            ...
            case Cubeside.BACK:
             ...
             //Billboard
            case Cubeside.DIAGFRONT:
                noCollVertices.Add(v7 + position);
                noCollVertices.Add(v6 + position);
                noCollVertices.Add(v2 + position);
                noCollVertices.Add(v3 + position);
                break;
            case Cubeside.DIAGBACK:
                noCollVertices.Add(v3 + position);
                noCollVertices.Add(v2 + position);
                noCollVertices.Add(v6 + position);
                noCollVertices.Add(v7 + position);
                break;
            case Cubeside.DIAGLEFT:
                noCollVertices.Add(v0 + position);
                noCollVertices.Add(v1 + position);
                noCollVertices.Add(v5 + position);
                noCollVertices.Add(v4 + position);
                break;
            case Cubeside.DIAGRIGHT:
                noCollVertices.Add(v4 + position);
                noCollVertices.Add(v5 + position);
                noCollVertices.Add(v1 + position);
                noCollVertices.Add(v0 + position);
        ...
        if (side == Cubeside.DIAGFRONT || side == Cubeside.DIAGBACK|| side == Cubeside.DIAGLEFT|| side == Cubeside.DIAGRIGHT)
                {
                    noCollUVs = new Vector2[] { uv11, uv01, uv00, uv10 };
                }
        else
        {
            uvs = new Vector2[]{new Vector2 (0,0.0625f), new Vector2(0.0625f,0.0625f),
                             new Vector2(0,0.125f), new Vector2(0.0625f,0.125f)};
        }
